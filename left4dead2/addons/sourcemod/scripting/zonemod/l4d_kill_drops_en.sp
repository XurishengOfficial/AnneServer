...
public OnPluginStart()
{
	...
	l4d_loot_boss_show_msg = CreateConVar("l4d_loot_boss_show_msg", "0", "0:disable print loot message in HintBox, 1:disable ", FCVAR_PLUGIN);
	...
	l4d_kill_addhp_lt50_mult = CreateConVar("l4d_kill_addhp_lt50_mult", "2.0", "health recover factor when health < 50", FCVAR_PLUGIN);
	l4d_kill_lowhp = CreateConVar("l4d_kill_lowhp", "25.0", "health recover factor when health < 20", FCVAR_PLUGIN);
	l4d_kill_addhp_healthlimit = CreateConVar("l4d_kill_addhp_healthlimit", "200.0", "health limit", FCVAR_PLUGIN);

	l4d_kill_show_msg = CreateConVar("l4d_kill_show_msg", "3", "print kill messge 1:in chat�2: int hintbox, 3:all, 0:disable", FCVAR_PLUGIN);
	...
	l4d_kill_addhp_weapon_other = CreateConVar("l4d_kill_addhp_weapon_other", "10.0", "health recover factor of ohter weapon killing", FCVAR_PLUGIN);
	...
	AutoExecConfig(true, "l4d_kill_drop_v22");
...
AddHealth(client, Float:add)
{
	...
	if(GetClientTeam(client) == 2) 
	{
		...
		new Float:newhp=add;
		new Float:lowhp=GetConVarFloat(l4d_kill_lowhp);
		if(fhp<lowhp)
		{
			newhp=add*(1.0+ (lowhp-fhp)*GetConVarFloat(l4d_kill_addhp_lt50_mult)/lowhp);
		}
...
public Action:Event_PlayerDeath(Handle:hEvent, const String:strName[], bool:DontBroadcast) {
	if(GetConVarInt(l4d_loot_enabled)==0 && GetConVarInt(l4d_kill_addhp_enabled) == 0)
	{
		return Plugin_Continue;
	}
	...
	new Float:dismult=1.0;

	new Float:weapon1=1.0;
	new Float:weapon2=1.0;
	new Float:weaponmult=1.0;

	new Float:addhpmult=GetConVarFloat(l4d_kill_addhp_mult);	// 0.15
	new difficult=1;
...
/////////////////////////////////////////////////////////////////////////
	...
	if(show)
	{
		...
		if(GetConVarInt(l4d_kill_addhp_enabled)>0)
		{
			...
			// kindmult=(fdist*(y2-y1)+y1*x2)/(x2-x1);
			// kindmult=y2-y1;
			// dismult: 1~2
			dismult=(fdist>x2-x1)?fdist/(x2-x1):1.0;
			dismult=dismult>1.5?1.5:dismult;
			kindmult=dismult*y1+y2-y1;
			...
			Format(hintmsg, sizeof(hintmsg),  "%s, %s, %s, kindmult = %2f, weaponmult = %2f, + %dhp", hintmsg, name, weapon, kindmult, weaponmult, difficult);
		}
 		...
 		if(ShowKillMsgOnChat())
		{
			...
			Format(chatmsg, sizeof(chatmsg),  "%s, %s, %s, kindmult = %2f, weaponmult = %2f, + %dhp", chatmsg, name, weapon, kindmult, weaponmult, difficult);
	...
	if(r < p)
	{
		...
		for(new i=0; i<num ; i++)
		{
			...
			if(r<a)
			{
				...
				// if(r1==0) Give(client, "defibrillator");
				// if(r1==1)	Give(client, "defibrillator");	
				// if(r1==3)	Give(client, "defibrillator");
				if(r1==4)	Give(client, "defibrillator");
			...
			else if(r<(a+b))
			{
				...
				if(r1==15) UpgradeLaserSight(client);
