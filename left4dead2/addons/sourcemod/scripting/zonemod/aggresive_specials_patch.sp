...
public void OnPluginStart()
{
	...
	g_cvEnable = CreateConVar("aggresive_specials_patch_enable", "1", _, FCVAR_NOTIFY);
	g_asEnable = CreateConVar("aggresive_specials_enable", "1", _, FCVAR_NOTIFY);
	g_saEnable = CreateConVar("specials_should_assault_enable", "0", _, FCVAR_NOTIFY);
	OnConVarChanged(null, "", "");
	g_cvEnable.AddChangeHook(OnConVarChanged);
	g_asEnable.AddChangeHook(OnConVarChanged);
	g_saEnable.AddChangeHook(OnConVarChanged);
}
