...
// Booleans
static bool b_HasPluginEnabled; // Used to state if the round started or not
...
// Handles
static ConVar h_HasPluginEnabled; // Related to the Boomer limit cvar
...
public void SetHostName()
{
	...
	if(FileExists(g_sPath))
	{
		...
		if(file)
		{
			while(!file.EndOfFile())
				file.ReadLine(g_HostNameBuffer, sizeof(g_HostNameBuffer));
			delete file;
			TrimString(g_HostNameBuffer);
		}
	}
	...
	{
		Format(g_HostNameBuffer, sizeof(g_HostNameBuffer), "%s [不藏特][%d特%d秒][训练模式]", g_HostNameBuffer, GetConVarInt(h_MaxPlayerZombies), GetConVarInt(h_InfectedSpawnTimeMax));
	}
	{
		Format(hostName, sizeof(hostName), "%s [%d特%d秒]", g_HostNameBuffer, GetConVarInt(h_MaxPlayerZombies), GetConVarInt(h_InfectedSpawnTimeMax));
	}
	h_HostName.SetString(hostName);
}
*/
...
public void OnPluginStart()
{
	...
	// console variables
	h_HasPluginEnabled = CreateConVar("l4d2_infectedbots_enable", "1", "是否开启答辩刷特", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	
	h_BoomerLimit = CreateConVar("l4d_infectedbots_boomer_limit", "1", "设定 Boomer 数量(胖子).", CVAR_FLAGS);
	h_SmokerLimit = CreateConVar("l4d_infectedbots_smoker_limit", "3", "设定 Smoker 数量(舌头).", CVAR_FLAGS);
	h_TankLimit = CreateConVar("l4d_infectedbots_tank_limit", "0", "设定 Tank 数量(持续刷坦克,慎用).", CVAR_FLAGS);
	if (b_IsL4D2)
	{
		h_SpitterLimit = CreateConVar("l4d_infectedbots_spitter_limit", "1", "设定 Spitter 数量(口水).", CVAR_FLAGS);
		h_JockeyLimit = CreateConVar("l4d_infectedbots_jockey_limit", "5", "设定 Jockey 数量(猴子).", CVAR_FLAGS);
		h_ChargerLimit = CreateConVar("l4d_infectedbots_charger_limit", "3", "设定 Charger 数量(牛牛).", CVAR_FLAGS);
	...
	h_MaxPlayerZombies = CreateConVar("l4d_infectedbots_max_specials", "8", "设定特感组总数量(越高越难).", CVAR_FLAGS); 
	h_InfectedSpawnTimeMax = CreateConVar("l4d_infectedbots_spawn_time_max", "0", "设定特感最大刷新间隔(秒).", CVAR_FLAGS);
	h_InfectedSpawnTimeMin = CreateConVar("l4d_infectedbots_spawn_time_min", "0", "设定特感最小刷新间隔(秒).", CVAR_FLAGS);
	...
	h_idletime_b4slay = CreateConVar("l4d_infectedbots_lifespan", "60", "多少秒后处死产生的特感(防止特感无限增多和长时间卡住不动占位置).", CVAR_FLAGS);
	h_InitialSpawn = CreateConVar("l4d_infectedbots_initial_spawn_timer", "5", "离开开始区域多少秒后才产生特感(建议不低于10秒).", CVAR_FLAGS);
	h_BotGhostTime = CreateConVar("l4d_infectedbots_ghost_time", "0", "设置大于0时,特感产生后为鬼魂状态(对抗,清道夫).", CVAR_FLAGS);
	h_DisableSpawnsTank = CreateConVar("l4d_infectedbots_spawns_disabled_tank", "0", "坦克出现时继续刷出特感? 0=继续, 1=停止.", CVAR_FLAGS);
	h_AdjustSpawnTimes = CreateConVar("l4d_infectedbots_adjust_spawn_times", "0", "0=使用游戏默认刷特方式, 1=根据游戏模式及幸存者数量自动调整特感数量(战役,对抗,清道夫).", CVAR_FLAGS);
	
	//AIMode_AutoAddBotCount
	AI_Enabled_Infectedbots = CreateConVar("l4d_ai_enabled_infectedbots", "0", "启用智能模式. 0=关闭, 1=开启.", CVAR_FLAGS);
	...
	HookConVarChange(h_HasPluginEnabled, ConVarPluginEnable);
	...
	// HookEvent("hostname_changed", Event_HostNameChanged, EventHookMode_Post);	
...
public void ConVarPluginEnable(ConVar convar, const char[] oldValue, const char[] newValue)
{
...
// bool checkFirstPlayerConnect(int client)
// {
// 	for (int i = 1; i < MaxClients + 1; i++)
