...
int    g_iEnabled, g_iDefaults, g_iHealthValue;
ConVar g_hEnabled, g_hDefaults, g_hHealthValue;

public Plugin myinfo =
{
	name = "l4d2_survivor_transition",
	author = "豆瓣酱な, Azuki", 
	description = "生还者过关时回复血量.",
...
public void OnPluginStart() 
{
	...
	g_hHealthValue	= CreateConVar("l4d2_survivor_transition_value", "50", "设置生还过关回血量.", FCVAR_NOTIFY, true, 1.0);
	
	g_hEnabled.AddChangeHook(l4d2survivorsConVarChanged);
	g_hDefaults.AddChangeHook(l4d2survivorsConVarChanged);
	g_hHealthValue.AddChangeHook(l4d2survivorsConVarChanged);
...
void l4d2_cvar_survivors()
{
	...
	g_iHealthValue = g_hHealthValue.IntValue;
}
...
void SetSurvivoHealth(int client)
{
	...
	{
		int survivorHealth = GetClientHealth(client);
		int tmpHealth = L4D_GetPlayerTempHealth(client);
		if (survivorHealth + tmpHealth < g_iHealthValue)
		{
